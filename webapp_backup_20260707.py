...
def render_mobile_invitations(user, query):
    ...
    for person in people:
        ...
        person_options.append(
            f'<option value="{person["id"]}" {selected}>{html.escape(person["name"])} ({role_label}{html.escape(class_text)})</option>'
        )
    ...
    <div class="panel">
      ...
      <form method="post" action="/mobile-invitations/create" class="user-grid" style="margin-top:12px">
        <div><label>Personne</label><select name="person_id" required>{''.join(person_options)}</select></div>
        <div><label>MESSAGE</label><input name="email" type="email" required placeholder="nom@example.com"></div>
        <div><label>Valide pendant</label>
          <select name="days">
            <option value="7">7 jours</option>
