...
def html_page(title, user, body, flash=None):
    ...
    if user:
        if user["role"] != "children":
            nav.append('<a class="nav-attendance" href="/mobile">Présence</a>')
        nav.append('<a href="/dashboard">Tableau</a>')
        if user["role"] == "children":
            nav.append('<a href="/agenda">Agenda</a>')
        else:
            nav.append('<a href="/agenda">AGENDA</a>')
            nav.append('<a href="/calendar">CALENDRIER</a>')
            nav.append('<a href="/allergic-children">ENFANTS ALLERGIQUES</a>')
        nav.append('<a href="/profile">Profil</a>')
        if user["role"] in {"principal", "boss"}:
            nav.append('<a href="/teacher-attendance">Présences éducatrices</a>')
            nav.append('<a href="/contacts">Contacts</a>')
        nav.append('<a href="/files">Fichiers</a>')
        nav.append('<a href="/mail">MESSAGE</a>')
        nav.append('<a href="/password-change">MOT DE PASSE</a>')
        if user["role"] in {"principal", "boss"}:
            nav.append('<a href="/reports">Fiche 4 semaines</a>')
        if user["role"] == "boss":
            nav.append('<a href="/audit">Journaux</a>')
        if user["role"] in MANAGE_CLOSED_DATES_ROLES:
            nav.append('<a href="/closed-dates">Dates fermées</a>')
        if user["role"] in MANAGE_USERS_ROLES:
            nav.append('<a href="/users">INVITATIONS</a>')
            nav.append('<a href="/children">Enfants</a>')
        nav.append(f'<form method="post" action="/logout" style="display:inline"><button type="submit">Déconnexion</button></form>')
    header = ""
    if user:
        ...
        header = f"""
        <div class="topbar">
          <div class="brand">Garderie Univers</div>
          <div class="nav">
            {''.join(nav)}
          </div>
          <div class="small">{html.escape(user['display_name'])} &middot; {ROLE_LABELS.get(user['role'], user['role'])}{presence_html}</div>
        </div>
        """
    else:
        header = """
    ...
    <head>
      ...
      <meta name="apple-mobile-web-app-title" content="Garderie Univers">
...
def creatable_roles_for_user(actor):
    if actor["role"] == "boss":
        return ["principal", "teacher", "cook", "children"]
    if actor["role"] == "principal":
...
def render_staff_calendar(user, query):
    ...
    for offset in range(28):
        ...
        value = 0 if chart_day_text in closed_dates else max(total_children - excluded_by_day.get(chart_day_text, 0), 0)
        chart_values.append((chart_day, value))
    chart_max = max([value for _day, value in chart_values] + [1])
    ...
    for index, (chart_day, value) in enumerate(chart_values):
        ...
        y = pad_t + plot_h - (plot_h * value / chart_max if chart_max else 0)
        chart_xy.append((x, y, chart_day, value))
    if chart_xy:
        if len(chart_xy) == 1:
            chart_path = f"M {chart_xy[0][0]:.1f} {chart_xy[0][1]:.1f}"
        else:
            parts = [f"M {chart_xy[0][0]:.1f} {chart_xy[0][1]:.1f}"]
            for index in range(len(chart_xy) - 1):
                x0, y0 = chart_xy[index][0], chart_xy[index][1]
                x1, y1 = chart_xy[index + 1][0], chart_xy[index + 1][1]
                cx0 = x0 + (x1 - x0) / 2
                cx1 = x1 - (x1 - x0) / 2
    ...
    chart_total_text = f"{chart_values[0][1]} demain" if chart_values else ""
...
def time_picker_html(name, value="", element_id="", css_class="teacher-time-input"):
    ...
    select_id = f' id="{html.escape(element_id)}"' if element_id else ""
    options = '<option value="">--:--</option>' + "".join(
        f'<option value="{hour:02d}:{minute:02d}" {"selected" if f"{hour:02d}:{minute:02d}" == selected_value else ""}>{hour:02d}:{minute:02d}</option>'
        for hour in range(24)
        for minute in range(60)
    )
    return f"""
      <div class="time-picker">
        <select{select_id} class="{html.escape(css_class)} time-picker-value" name="{html.escape(name)}" aria-label="Time">{options}</select>
      </div>
    """
...
def render_teacher_attendance(user, query):
    ...
    for row in recent_rows:
        ...
        recent_cells.append(
            ...
            f"<td>{html.escape(EVENT_LABELS.get(row['event_type'], row['event_type']))}</td>"
            f"<td>{html.escape(row['actor_name'] or 'System')}</td>"
            f"<td>{html.escape(source_label)}</td>"
            f"<td>{html.escape(device_name)}</td>"
            f"<td>{snapshot_html or '-'}</td>"
            f"</tr>"
        )
    recent_html = "".join(recent_cells)
    ...
    if action_mode == "pay_hours":
        ...
        <form method="get" action="/teacher-attendance" class="pay-hours-form">
          ...
          <button class="btn primary" type="submit">PAY HOURS</button>
        </form>
        <div class="table-wrap pay-hours-table">
          <table>
            <thead><tr><th>Teacher</th><th>Total</th>{pay_day_headers}<th>Days</th><th>Events</th><th>Open</th></tr></thead>
            <tbody>{pay_rows_html or f'<tr><td colspan="{5 + len(pay_days)}" class="muted">No teachers found.</td></tr>'}</tbody>
          </table>
        </div>
        """
    load_schedule_html = ""
    if action_mode == "load_schedule":
    ...
    if user["role"] == "boss":
        teacher_action_buttons_html = f"""
          <div class="teacher-action-buttons">
            <a class="btn secondary" href="/teacher-attendance?action=load_schedule&date={quote(selected_date)}">LOAD SCHEDULE</a>
            <a class="btn primary" href="/teacher-attendance?action=pay_hours&date={quote(selected_date)}&pay_start={quote(pay_start)}&pay_end={quote(pay_end)}">PAY HOURS</a>
          </div>
          {load_schedule_html}
          {pay_hours_html}
        """
    body = f"""
    <style>
      .teacher-attendance-head {{ display:flex; align-items:end; justify-content:space-between; gap:12px; flex-wrap:wrap; }}
      ...
      @media (max-width:720px) {{
        .teacher-attendance-head, .teacher-attendance-date {{ display:grid; grid-template-columns:1fr; }}
    ...
    <div class="panel teacher-status-panel" style="margin-top:16px">
    ...
    <div class="panel" style="margin-top:16px">
      ...
      <div class="table-wrap">
        <table>
          <thead><tr><th>Timestamp</th><th>Teacher</th><th>Event</th><th>By</th><th>Source</th><th>Device</th><th>Photo</th></tr></thead>
          <tbody>{recent_html or '<tr><td colspan="7" class="muted">No teacher attendance records for this date.</td></tr>'}</tbody>
        </table>
      </div>
    </div>
    <script>
...
def render_users(user, query):
    ...
    with connect_db() as conn:
        users = conn.execute("SELECT * FROM web_users").fetchall()
        ...
        invite_people = conn.execute(
            ...
            WHERE role IN ('children', 'teachers')
              AND id NOT IN (SELECT person_id FROM web_users WHERE person_id IS NOT NULL)
    ...
    for target in users:
        ...
        table_rows.append(
            ...
            <tr>
              ...
              <td>
                <a class="btn" href="/users/edit?id={target['id']}">Modifier</a>
                {f'<a class="btn green" href="/users?invite_user_id={target["id"]}#invite-user">Inviter</a>' if can_invite_user else ''}
                {reset_device_button}
                {delete_button}
              </td>
            </tr>
    ...
    <div class="panel">
      ...
      <div class="btn-row" style="margin-top:10px">
        <a class="btn {'primary' if sort_key == 'name' else 'ghost'}" href="/users?sort=name">Sort Name</a>
        <a class="btn {'primary' if sort_key == 'role' else 'ghost'}" href="/users?sort=role">Sort Role</a>
        <a class="btn {'primary' if sort_key == 'classes' else 'ghost'}" href="/users?sort=classes">Sort Classes</a>
        <a class="btn {'primary' if sort_key == 'device' else 'ghost'}" href="/users?sort=device">Sort Device</a>
      </div>
      <div class="table-wrap" style="margin-top:12px">
        <table>
          <thead><tr><th>Username</th><th>Name</th><th>Role</th><th>Classes</th><th>Mobile device</th><th>Password changed</th><th>Action</th></tr></thead>
          <tbody>{''.join(table_rows) or '<tr><td colspan=\"7\" class=\"muted\">No users</td></tr>'}</tbody>
        </table>
      </div>
    </div>
    <div class="panel" id="invite-user" style="margin-top:16px">
    ...
    <div class="panel" style="margin-top:16px">
      ...
      <form method="post" action="/users/create" class="user-grid">
        ...
        <div><label>Role</label>
          <select name="role">
            {"".join(f"<option value='{role}'>{ROLE_LABELS.get(role, role)}</option>" for role in creatable_roles_for_user(user))}
          </select>
        </div>
        ...
        <div style="display:flex;align-items:end"><button class="btn primary" type="submit">Créer</button></div>
      </form>
    </div>
    """
...
def render_children_admin(user, query):
    ...
    rows = "".join(
        ...
        <tr>
          ...
          <td class="nowrap">
            <a class="btn" href="/children/edit?id={child['id']}">Modifier</a>
            <a class="btn green" href="/users?invite_person_id={child['id']}#invite-user">Inviter</a>
    ...
    <div class="panel">
      ...
      <div class="table-wrap" style="margin-top:12px">
        <table>
          <thead><tr><th>Name</th><th>Groupe</th><th>Photo path</th><th>Created</th><th>Action</th></tr></thead>
    ...
    <div class="panel" style="margin-top:16px">
      ...
      <form method="post" action="/children/create" class="user-grid">
        ...
        <div><label>Groupe</label><select name="class_name">{class_options}</select></div>
...
def render_audit(user):
    ...
    <div class="panel">
      ...
      <div class="table-wrap">
        <table>
          <thead><tr><th>Time</th><th>Actor</th><th>Device</th><th>IP Address</th><th>Action</th><th>Type</th><th>Object</th><th>Details</th></tr></thead>
...
def send_mobile_invitation_email(to_address, person_name, invite_url, expires_at):
    subject = "Invitation mobile - Garderie Univers"
    body = f"""Bonjour,

Vous avez reçu une invitation pour créer votre compte mobile Garderie Univers.
...
def render_mobile_install(user):
    ...
    <div class="panel" style="max-width:640px;margin:0 auto">
      <h2>Installer Garderie Univers</h2>
      <p class="muted">Ajoutez cette application à l'écran d'accueil du téléphone. La prochaine fois, ouvrez l'icône Garderie Univers sans saisir l'adresse du serveur.</p>
    ...
    def do_POST(self):
        ...
        if path == "/mail/send":
            ...
            if not recipient_values or not subject or not body_text:
                self.send_html(render_mail(user, {}, error="MESSAGE recipient, subject, and message are required."), status=200)
                return
            with connect_db() as conn:
                allowed = {}
        ...
        if path == "/users/delete":
            ...
            with connect_db() as conn:
                ...
                conn.execute("DELETE FROM user_files WHERE owner_user_id = ? OR uploader_user_id = ?", (target_user_id, target_user_id))
        ...
        if path == "/users/update":
            ...
            with connect_db() as conn:
                ...
                if self_edit:
                    ...
                    audit_request(self, conn, user["id"], "update_user_password", "user", object_id=target["id"], details={"username": target["username"]})
                    conn.commit()
                    redirect(self, "/users")
                    return
                username = form.get("username", [""])[0].strip()
        ...
        if path == "/children/create":
            ...
            with connect_db() as conn:
                ...
                audit_request(self, conn, user["id"], "create_child", "child", object_id=child_id, details={"name": name, "class_name": class_name})
                conn.commit()
        ...
        if path == "/children/delete":
            ...
            with connect_db() as conn:
                ...
                audit_request(self, conn, user["id"], "delete_child", "child", object_id=child_id, details={"name": child["name"]})
                conn.commit()
            redirect(self, "/children")
            return
