...
def css():
    ...
    * { box-sizing: border-box; }
    ...
    .menu-toggle { width: 40px; height: 40px; border: 0; border-radius: 6px; background: rgba(255,255,255,0.16); color: var(--navText); font-size: 24px; line-height: 1; cursor: pointer; display: inline-flex; align-items: center; justify-content: center; }
...
def html_page(title, user, body, flash=None):
    ...
    if user:
        ...
        if user["role"] in MANAGE_USERS_ROLES:
            nav_left.append('<a href="/users">Account et inviter</a>')
        if user["role"] == "boss":
...
def login_page(error=None, show_project_register=True):
    project_register_html = (
        '<div style="margin-top:12px"><a href="/project/register">Créer un nouveau projet</a></div>'
...
def render_project_register(error=None, created_url=None):
    ...
    if created_url:
        ...
        <div class="alert info">
          Projet créé. Lien d'inscription du premier propriétaire:
          <div style="margin-top:6px"><a href="{html.escape(created_url)}">{html.escape(created_url)}</a></div>
        </div>
        """
    body = f"""
    <div class="login-box">
      <h1 style="margin-top:0">Créer un projet</h1>
      {created_html}
      {f'<div class="alert error">{html.escape(error)}</div>' if error else ''}
      <form method="post" action="/project/register" class="grid" style="gap:12px">
        <div>
          <label>Nom du projet / garderie</label>
      ...
      <div style="margin-top:12px"><a href="/">Retour connexion</a></div>
    </div>
    """
...
def render_contacts(user, query=None):
    ...
    rows = sorted(
        rows,
        key=lambda row: (
            (ROLE_LABELS.get(row["role"], row["role"]) if sort_key == "role" else (row["display_name"] or row["username"] or "")).lower(),
...
def render_users(user, query):
    ...
    <style>
      ...
      .users-records-scroll {{ max-height: 166px; overflow-y: auto; }}
      .invite-records-scroll table {{ margin: 0; }}
      .users-records-scroll table {{ margin: 0; width:100%; min-width:860px; table-layout:fixed; }}
      ...
      .users-compact-table th, .users-compact-table td {{ padding-left:8px; padding-right:8px; vertical-align:middle; overflow:hidden; text-overflow:ellipsis; }}
      .account-col-select {{ width:42px; }}
      .account-col-username {{ width:15%; }}
      .account-col-name {{ width:18%; }}
      .account-col-role {{ width:12%; }}
      .account-col-classes {{ width:14%; }}
      .account-col-device {{ width:17%; }}
      .account-col-password {{ width:16%; }}
      .account-col-action {{ width:8%; }}
...
def render_user_edit(user, target_id):
        ...
        if target["role"] == "children":
            if target["person_id"]:
                child_person = conn.execute(
                    "SELECT id, class_name FROM persons WHERE id = ? AND role = 'children' AND project_id = ?",
    ...
    <div class="panel">
      ...
      <form method="post" action="/users/update" class="user-grid">
        <input type="hidden" name="id" value="{target['id']}">
        <input type="hidden" name="username" value="{html.escape(username_value)}">
        <input type="hidden" name="display_name" value="{html.escape(display_name_value)}">
        <input type="hidden" name="role" value="{html.escape(role_value)}">
        <input type="hidden" name="person_id" value="{html.escape(person_id_value)}">
        <div><label>Groupe</label>
          <select name="class_name">
            {child_group_options}
          </select>
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if not user:
            ...
            if path == "/":
                self.send_html(login_page(show_project_register=not hide_project_register))
                return
            self.send_html(login_page(show_project_register=not hide_project_register))
            return
        ...
        if path == "/projects/delete":
            ...
            return
        if user["role"] == "children" and path in {"/reports", "/statistics", "/statistics/15min", "/teacher-attendance", "/export.xlsx", "/contacts", "/contacts.xlsx", "/users/export.xlsx", "/closed-dates", "/users", "/children", "/children/cards", "/mobile-invitations", "/audit", "/calendar", "/allergic-children"}:
            redirect(self, "/dashboard")
            return
        if path == "/dashboard":
            if user["role"] == "children":
    ...
    def do_POST(self):
        ...
        if path == "/children/create":
            ...
            redirect(self, f"{return_to}?account_flash={quote(flash_token)}")
            return
