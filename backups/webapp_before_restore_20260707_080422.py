...
def get_classes(conn):
    hidden = {
        row["name"]
        for row in conn.execute("SELECT name FROM hidden_class_names").fetchall()
    }
    try:
        rows = conn.execute("SELECT name FROM class_names ORDER BY name COLLATE NOCASE").fetchall()
        classes = [row["name"] for row in rows if row["name"] and row["name"] not in hidden]
    except sqlite3.OperationalError:
        classes = []
    if not classes:
        rows = conn.execute(
            "SELECT DISTINCT class_name FROM persons WHERE role = 'children' AND class_name <> '' ORDER BY class_name COLLATE NOCASE"
        ).fetchall()
        classes = [row["class_name"] for row in rows if row["class_name"] and row["class_name"] not in hidden]
    return classes
...
def store_pending_child_list_import(user_id, filename, entries):
...
def apply_child_list_entries(conn, user_id, filename, entries):
    created = 0
    updated = 0
    skipped = 0
    for child_name, group_name in entries:
        ...
        if not child_name:
            ...
            continue
        if group_name:
            conn.execute("INSERT OR IGNORE INTO class_names(name) VALUES (?)", (group_name,))
        existing = conn.execute(
            "SELECT * FROM persons WHERE role = 'children' AND lower(name) = lower(?)",
            (child_name,),
        ).fetchone()
        if existing:
            ...
            conn.execute(
                ...
                VALUES (?, 'children', ?, ?, ?, ?)
                """,
                (child_name, group_name, photo_path, f"CHILD:{child_name}", now_text()),
            )
            created += 1
    audit(
        conn,
...
def teacher_schedule_for_day(conn, teacher_name, day_text):
    rows = conn.execute(
        """
        SELECT schedule_in, schedule_out, work_hours, class_name
        FROM teacher_schedule
        WHERE lower(teacher_name) = lower(?) AND day_text = ?
        ORDER BY schedule_in ASC, schedule_out ASC, id ASC
        """,
        (teacher_name, day_text),
    ).fetchall()
    if not rows:
...
def calculate_teacher_work_hours(day_text, first_checkin, last_checkout, schedule_in):
    if not first_checkin or not last_checkout or not schedule_in:
        return ""
    try:
...
def get_attendance_rows(conn, person_id, day_text):
    ...
    return conn.execute(
        ...
        FROM attendance
        WHERE lower(name) = lower(?) AND role = ? AND timestamp LIKE ?
        ORDER BY timestamp ASC, id ASC
        """,
        (person["name"], person["role"], f"{day_text}%"),
    ).fetchall()
...
def dashboard_arrival_increase_points(conn, children, day_text):
    ...
    try:
        ...
        return []
    child_names = [str(child["name"]).strip().lower() for child in children if str(child["name"]).strip()]
    if not child_names:
        return []
    placeholders = ",".join("?" for _ in child_names)
    rows = conn.execute(
        f"""
        SELECT lower(name) AS person_key, event_type, timestamp
        FROM attendance
    ...
    while slot_time <= end_time:
        while row_index < len(rows):
            ...
            if rows[row_index]["event_type"] == "checkin":
                current_children.add(rows[row_index]["person_key"])
            elif rows[row_index]["event_type"] == "checkout":
                current_children.discard(rows[row_index]["person_key"])
            row_index += 1
...
def css():
    ...
    * { box-sizing: border-box; }
    ...
    a { color: #3f648e; text-decoration: none; }
    a:hover { text-decoration: underline; }
    .topbar { position: sticky; top: 0; z-index: 1000; background: var(--nav); color: var(--navText); padding: 12px 20px; display: flex; align-items: center; justify-content: space-between; gap: 12px; flex-wrap: wrap; box-shadow: 0 8px 24px rgba(21, 34, 56, 0.16); }
    .brand { font-size: 23px; font-weight: 900; letter-spacing: 0; white-space: nowrap; background: linear-gradient(90deg, #e11d48, #f97316, #eab308, #22c55e, #06b6d4, #2563eb, #a855f7, #e11d48); background-size: 240% 100%; -webkit-background-clip: text; background-clip: text; color: transparent; animation: brand-rainbow 5s linear infinite; text-shadow: 0 1px 1px rgba(255,255,255,0.45); }
    ...
    .menu-toggle { width: 40px; height: 40px; border: 0; border-radius: 6px; background: rgba(255,255,255,0.42); color: var(--navText); font-size: 24px; line-height: 1; cursor: pointer; display: inline-flex; align-items: center; justify-content: center; }
    .menu-toggle:hover { background: rgba(255,255,255,0.68); }
    .account-toggle { border: 1px solid rgba(116,143,171,0.22); border-radius: 6px; background: rgba(255,255,255,0.48); color: var(--navText); min-height: 40px; padding: 7px 11px; cursor: pointer; font-weight: 700; display: inline-flex; align-items: center; gap: 10px; }
    ...
    .side-drawer { position: fixed; top: 12px; left: 12px; max-height: calc(100vh - 24px); width: min(220px, calc(100vw - 24px)); z-index: 2200; display: none; overflow-y: auto; background: #fff; border: 1px solid var(--line); border-radius: 10px; box-shadow: 0 22px 60px rgba(15,23,42,0.28); padding: 8px; }
    .side-drawer.show { display: block; }
    .side-drawer .nav { display: grid; grid-template-columns: 1fr; gap: 4px; width: 100%; }
    .side-drawer .nav a, .side-drawer .nav button { width: 100%; justify-content: flex-start; text-align: left; background: #fff; border: 0; color: var(--text); min-height: 46px; font-size: 15px; padding: 10px 12px; }
    .side-drawer .nav a:hover, .side-drawer .nav button:hover { background: #eef6ff; }
    .account-popover { position: fixed; top: 62px; right: 14px; max-height: calc(100vh - 76px); width: min(150px, calc(100vw - 28px)); z-index: 2200; display: none; overflow-y: auto; background: #fff; border: 1px solid var(--line); border-radius: 10px; box-shadow: 0 20px 54px rgba(15,23,42,0.24); padding: 8px; }
    .account-popover.show { display: block; }
    .account-popover .nav { display: grid; grid-template-columns: 1fr; gap: 4px; width: 100%; }
    .account-popover .nav a, .account-popover .nav button { width: 100%; justify-content: flex-start; text-align: left; min-height: 44px; background: #fff; border: 0; color: var(--text); padding: 10px 12px; }
    .account-popover .nav a:hover, .account-popover .nav button:hover { background: #eef6ff; }
    .nav a, .nav button { color: var(--navText); background: rgba(255,255,255,0.42); border: 1px solid rgba(116, 143, 171, 0.22); padding: 7px 11px; border-radius: 6px; cursor: pointer; font-size: 13px; text-decoration: none; transition: background 120ms ease, border-color 120ms ease, box-shadow 120ms ease; }
    .nav a:hover, .nav button:hover { background: rgba(255,255,255,0.68); border-color: rgba(116, 143, 171, 0.34); text-decoration: none; }
    .nav a.nav-attendance { background: #d7f3e2; border-color: #bce6cf; color: #24543b; font-weight: 800; }
    ...
    input:focus, select:focus, textarea:focus { border-color: #8bb4df; box-shadow: 0 0 0 3px var(--focus); }
    ...
    .btn.primary { background: #cfe3ff; border-color: #b7d4f5; color: #244463; }
    .btn.green { background: #d7f3e2; border-color: #bce6cf; color: #24543b; }
    .btn.gray { background: #e6ebf2; color: #394b59; }
    .btn.red { background: #f8d6da; border-color: #efbdc3; color: #6f2c32; }
    .btn.amber { background: #f8e7bd; border-color: #efd69b; color: #654b1f; }
    ...
    .dashboard-overview { display: grid; grid-template-columns: minmax(560px, auto) minmax(720px, 1fr); gap: 28px; align-items: stretch; }
    .dashboard-charts { display: grid; grid-template-columns: repeat(2, minmax(320px, 1fr)); gap: 16px; }
    .dashboard-chart { min-height: 190px; border: 1px solid #e3ebf4; border-radius: 8px; background: #fbfdff; padding: 10px 12px 8px; }
    .dashboard-chart-head { display: flex; align-items: center; justify-content: space-between; gap: 12px; margin-bottom: 2px; }
    .dashboard-chart-title { font-size: 13px; font-weight: 700; color: #26384d; }
    .dashboard-chart-total { font-size: 12px; font-weight: 700; color: #5f7185; }
    .arrival-chart { width: 100%; height: 170px; display: block; }
    .arrival-chart text { fill: #6d7c8d; font-size: 10px; }
    .chart-grid line { stroke: #e5edf5; stroke-width: 1; }
    .chart-area { fill: rgba(82, 152, 219, 0.12); }
    .chart-line { fill: none; stroke: #3f7fb8; stroke-width: 2.5; stroke-linecap: round; stroke-linejoin: round; }
    .chart-points { display: none; }
    .chart-points circle { fill: #fff; stroke: #3f7fb8; stroke-width: 2; }
    .presence-line .chart-area { fill: rgba(67, 160, 111, 0.12); }
    .presence-line .chart-line { stroke: #3f9b6a; }
    .presence-line .chart-points circle { stroke: #3f9b6a; }
    ...
    .card.selected { outline: 2px solid #1d4ed8; outline-offset: 1px; border-color: #1d4ed8; }
    ...
    .file-actions { position: absolute; top: 8px; right: 8px; display: flex; gap: 4px; flex-wrap: wrap; margin-top: 0; align-items: center; justify-content: flex-end; }
    .file-actions .btn { min-height: 24px; padding: 2px 7px; font-size: 11px; line-height: 1; border-radius: 4px; }
    ...
    @media (max-width: 720px) {
      ...
      .side-drawer .nav a, .side-drawer .nav button, .account-popover .nav a, .account-popover .nav button { text-align: left; justify-content: flex-start; font-size: 14px; }
      ...
      .fiche-week { grid-template-columns: 1fr; }
      .fiche-week-label { flex-direction: row; justify-content: space-between; align-items: center; }
      .fiche-day { min-height: 48px; padding: 5px; }
      ...
      .file-actions { position: static; margin-top: 6px; justify-content: stretch; display: grid; grid-template-columns: 1fr 1fr; }
      .file-actions .btn { min-height: 34px; font-size: 12px; padding: 6px 8px; }
      .mail-folder-files { max-height: 120px; }
      .mail-target-list, .mail-send-to { min-height: 96px; max-height: 180px; }
      .mail-target-row, .mail-target-head { grid-template-columns: minmax(64px, 0.7fr) minmax(92px, 1fr) minmax(130px, 1.4fr); gap: 6px; padding: 6px 8px; }
...
def html_page(title, user, body, flash=None):
    ...
    nav_right = []
    if user:
        nav_left.append('<a href="/dashboard">Tableau</a>')
        if user["role"] == "children":
        ...
        if user["role"] in MANAGE_USERS_ROLES:
            nav_left.append('<a href="/users">Account</a>')
            nav_left.append('<a href="/children">Enfents liste</a>')
        if user["role"] != "children":
    ...
    if user:
        ...
        <div class="topbar">
          <div class="topbar-main">
            <div class="brand">Garderie L'Univers De Cassiopée</div>
          </div>
    ...
    <head>
      ...
      <meta name="apple-mobile-web-app-title" content="Garderie L'Univers De Cassiopée">
...
def render_password_reset(error=None, success=None):
    ...
    <div class="login-box">
      ...
      <p class="muted">Entrez votre nom d'utilisateur et un téléphone ou E-MAIL déjà enregistré dans votre profil.</p>
      {f'<div class="alert error">{html.escape(error)}</div>' if error else ''}
      {f'<div class="alert info">{html.escape(success)}</div>' if success else ''}
      <form method="post" action="/password-reset" class="grid" style="gap:12px">
        <div>
          <label>Username</label>
          <input name="username" autocomplete="username" required>
        </div>
        <div>
          <label>Téléphone ou E-MAIL enregistré</label>
          <input name="recovery_contact" required>
        </div>
        <div>
...
def classes_for_user(user, conn):
    ...
    if user["role"] in VIEW_ALL_CLASSES_ROLES:
        return all_classes
    allowed = json.loads(user["allowed_classes_json"] or "[]")
    if user["role"] == "teacher" and not allowed:
        return all_classes
...
def create_or_reuse_person_for_user_role(conn, role, display_name, class_name="", photo_path=""):
    ...
    if existing:
        if role == "children" and class_name and not (existing["class_name"] or "").strip():
            ...
            conn.execute("INSERT OR IGNORE INTO class_names(name) VALUES (?)", (class_name,))
        return existing["id"]
    resolved_photo_path = (photo_path or "").strip()
    resolved_class_name = (class_name or "").strip()
    if role == "children":
        if resolved_class_name:
            conn.execute("INSERT OR IGNORE INTO class_names(name) VALUES (?)", (resolved_class_name,))
    ...
    conn.execute(
        ...
        VALUES (?, ?, ?, ?, ?, ?)
        """,
        (name, person_role, resolved_class_name, resolved_photo_path, qr_token, now_text()),
    )
    return conn.execute("SELECT last_insert_rowid()").fetchone()[0]


def create_or_reuse_person_with_role(conn, role, display_name, class_name="", photo_path=""):
    ...
    conn.execute(
        ...
        VALUES (?, ?, ?, ?, ?, ?)
        """,
        (name, role, resolved_class_name, resolved_photo_path, qr_token, now_text()),
    )
    return conn.execute("SELECT last_insert_rowid()").fetchone()[0]
...
def render_mobile_dashboard(user, query):
        ...
        if selected_child:
            ...
            selected_child_panel = f"""
              <section class="mobile-child {status_class}" id="mobile-selected-child">
                ...
                {actions}
                <a class="btn ghost" href="{list_url}" style="width:100%;margin-top:2px">Retour à la liste</a>
              </section>
            """
    ...
    <style>
      ...
      .mobile-child.selected {{ border-color:#1d4ed8; box-shadow:0 0 0 2px rgba(29,78,216,0.12); }}
      .mobile-child.present {{ border-color:#9ad9af; background:#f3fbf6; }}
      ...
      .mobile-photo {{ overflow:hidden; background:#eef6ff; border:1px solid #d7e4f2; display:flex; align-items:center; justify-content:center; }}
      ...
      .mobile-btn.arrive {{ background:#d7f3e2; border-color:#bce6cf; color:#24543b; }}
      .mobile-btn.depart {{ background:#dbe7f5; border-color:#c5d4e5; color:#2c465f; }}
...
def render_staff_calendar(user, query):
    ...
    with connect_db() as conn:
        ...
        if person_ids:
            ...
            rows = conn.execute(
                f"""
                SELECT child_calendar_events.day_text, COUNT(DISTINCT web_users.person_id) AS child_count
                ...
                GROUP BY child_calendar_events.day_text
                """,
                [*person_ids, query_start.strftime("%Y-%m-%d"), query_end.strftime("%Y-%m-%d")],
            ).fetchall()
            excluded_by_day = {row["day_text"]: int(row["child_count"] or 0) for row in rows}
    closed_dates = set(load_closed_dates())
    ...
    planned_chart_html = f"""
      <div class="dashboard-chart" style="min-height:230px;max-width:460px;margin-left:70px">
        <div class="dashboard-chart-head">
    ...
    <div class="panel">
      <h2>CALENDRIER</h2>
      <form method="get" action="/calendar" class="toolbar">
        <div>
          <label>Mois</label>
        ...
        <div style="align-self:end" class="btn-row">
      ...
      <div style="display:grid;grid-template-columns:auto minmax(360px,1fr);gap:28px;align-items:start">
        <div class="table-wrap">
    ...
    with connect_db() as conn:
        ...
        for child in children:
            ...
            latest = conn.execute(
                ...
                SELECT timestamp
                FROM attendance
                WHERE lower(name) = lower(?) AND role = 'children' AND timestamp LIKE ?
                ORDER BY timestamp DESC, id DESC
                LIMIT 1
                """,
                (child["name"], f"{selected_date}%"),
            ).fetchone()
            rows.append({
                "child": child,
                "allergies": allergies,
    ...
    for row in rows:
        ...
        cards.append(
            ...
            <div class="panel" style="padding:12px">
              ...
              <div class="muted-box">
                <div style="font-weight:800;margin-bottom:6px">Alimentaire ALLERGIES</div>
...
def render_teacher_mobile_dashboard(user):
    with connect_db() as conn:
        linked_person = None
        if user["person_id"]:
            linked_person = conn.execute("SELECT * FROM persons WHERE id = ? AND role = 'teachers'", (user["person_id"],)).fetchone()
        status = mobile_person_status(conn, user)
        location_policy = attendance_location_payload(conn)
        schedule = teacher_schedule_for_day(conn, linked_person["name"], today_text()) if linked_person else {"schedule_in": "", "schedule_out": ""}
        has_reference_faces = bool(reference_face_paths_for_person(linked_person["name"])) if linked_person else False
    if not linked_person:
        body = """
        <div class="panel" style="max-width:560px;margin:0 auto">
          <h2>Présence mobile</h2>
        ...
        """
        return html_page("Présence mobile", user, body)
    ...
    <style>
      ...
      .teacher-status {{ display:grid; grid-template-columns:1fr auto; gap:10px; align-items:center; }}
      .teacher-status strong {{ font-size:24px; color:#24476f; }}
      .teacher-camera {{ width:min(100%, 260px); aspect-ratio:1/1; margin:0 auto; display:block; background:#e8f1f8; border:2px solid #9db7d3; border-radius:999px; object-fit:cover; }}
      .teacher-canvas {{ display:none; }}
      .teacher-actions {{ display:grid; grid-template-columns:1fr 1fr; gap:10px; }}
      .teacher-actions .btn {{ min-height:54px; font-size:17px; }}
      ...
      @media (max-width:560px) {{ .wrap {{ padding:12px; }} .teacher-actions {{ grid-template-columns:1fr; }} }}
    </style>
    <div class="teacher-mobile-shell">
      <div class="panel">
        ...
        <div class="teacher-status">
          <div>
            ...
            <strong id="teacher-status">{html.escape(status_text)}</strong>
            <div class="small muted">Dernier: <span id="teacher-last">{html.escape(last_text)}</span></div>
          </div>
          <a class="btn ghost" href="/mobile">Actualiser</a>
        </div>
      ...
      <div class="panel">
        ...
        <div class="teacher-actions" style="margin-top:12px">
          ...
          <button class="btn" type="button" data-event-type="checkout" {"disabled" if not is_present else ""}>Départ visage</button>
        </div>
        <button class="btn amber" type="button" id="enroll-face-button" style="width:100%;margin-top:10px">Enregistrer le visage</button>
    ...
    (function() {{
      ...
      startCamera().then(function() {{ setMessage('Caméra prête.'); }}).catch(function(error) {{ setMessage(error.message, 'error'); }});
    }})();
    </script>
    """
    return html_page("Présence mobile", user, body)
...
def render_dashboard(user, query):
    with connect_db() as conn:
        ...
        for child in children:
            ...
            cards.append(
                f"""
                <a class="card-link" href="/dashboard?class={quote(selected_class)}&date={quote(selected_date)}&sort={quote(selected_sort)}&child_id={child['id']}#selected-child-actions">
        ...
        if user["role"] in {"principal", "boss"}:
            ...
            <div class="panel">
              <div class="btn-row">
                <a class="btn amber" href="{stats_url}">Résumé des présences</a>
                <a class="btn" href="{export_url}">Exporter Excel</a>
                <a class="btn primary" href="{fiche_url}">Fiche 4 semaines</a>
              </div>
            </div>
            """
        ...
        """
        body = f"""
        <div class="{'child-dashboard' if is_child_account else ''}">
...
def render_contacts(user):
    ...
    body = f"""
    <div class="panel">
      <h2>Phone and MESSAGE List</h2>
      <div class="toolbar">
        <a class="btn primary" href="/contacts.xlsx">Exporter Excel</a>
      </div>
      <div class="table-wrap">
        <table>
          <thead><tr><th>Name</th><th>Role</th><th>Phones</th><th>MESSAGE</th></tr></thead>
...
def teacher_attendance_day_summary(conn, day_text):
    teachers = conn.execute(
        """
    ...
    for teacher in teachers:
        events = conn.execute(
            ...
            FROM attendance
            WHERE lower(name) = lower(?) AND role = 'teachers' AND timestamp LIKE ?
            ORDER BY timestamp ASC, id ASC
            """,
            (teacher["name"], f"{day_text}%"),
        ).fetchall()
        ...
        if checkins:
            ...
            not_arrived += 1
        schedule = teacher_schedule_for_day(conn, teacher["name"], day_text)
...
def teacher_pay_hours_summary(conn, start_date_text, end_date_text):
    try:
...
def teacher_six_week_hours_summary(conn, end_date_text):
    try:
        end_date = datetime.strptime(end_date_text, "%Y-%m-%d").date()
    except ValueError:
    ...
    for offset in range(5, -1, -1):
        ...
        _start, _end, _days, rows = teacher_pay_hours_summary(conn, start.strftime("%Y-%m-%d"), end.strftime("%Y-%m-%d"))
        weeks.append(
            {
...
def upsert_teacher_schedule_times(conn, actor, handler, teacher, day_text, schedule_in_value, schedule_out_value):
    ...
    if not schedule_in_value and not schedule_out_value:
        return False
    existing = teacher_schedule_for_day(conn, teacher["name"], day_text)
    ...
    conn.execute(
        "DELETE FROM teacher_schedule WHERE lower(teacher_name) = lower(?) AND day_text = ?",
        (teacher["name"], day_text),
    )
    conn.execute(
        """
        INSERT INTO teacher_schedule(
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        """,
        (teacher["name"], day_text, schedule_in, schedule_out, work_hours, class_name, "manual_edit", now_text()),
    )
...
def render_teacher_attendance(user, query):
    ...
    with connect_db() as conn:
        summary = teacher_attendance_day_summary(conn, selected_date)
        ...
        pay_start, pay_end, pay_days, pay_rows = teacher_pay_hours_summary(conn, pay_start, pay_end) if action_mode == "pay_hours" else (pay_start, pay_end, [], [])
        trend_weeks = teacher_six_week_hours_summary(conn, selected_date)
    ...
    for teacher in summary["teachers"]:
        ...
        first_in_picker = time_picker_html(f"first_in_{teacher['id']}", teacher["first_checkin"][11:16] if teacher["first_checkin"] else "")
        last_out_picker = time_picker_html(f"last_out_{teacher['id']}", teacher["last_checkout"][11:16] if teacher["last_checkout"] else "")
        schedule_in_picker = time_picker_html(f"schedule_in_{teacher['id']}", teacher["schedule_in"])
        schedule_out_picker = time_picker_html(f"schedule_out_{teacher['id']}", teacher["schedule_out"])
        teacher_rows.append(
            f"""
            <tr data-teacher-name="{html.escape(teacher['name'])}" data-original-work-hours="{html.escape(teacher['work_hours'] or '0')}">
              ...
              <td>{schedule_out_picker}</td>
              <td class="work-hours-cell">{html.escape(teacher['work_hours'])}</td>
            </tr>
            """
        )
    ...
    if user["role"] == "boss":
        teacher_action_buttons_html = f"""
          ...
          {pay_hours_html}
        """
    teacher_trend_chart = teacher_hours_trend_chart_html(trend_weeks)
    recent_records_html = ""
    if user["role"] == "boss":
    ...
    <style>
      ...
      .teacher-hours-chart {{ border:1px solid #e3ebf4; border-radius:8px; background:#fbfdff; padding:8px 12px 4px; max-width:470px; transform:translate(-100px, 100px); }}
      .teacher-hours-chart .arrival-chart {{ height:145px; }}
      .teacher-actual-line {{ stroke:#3f7fb8; }}
      .teacher-schedule-line {{ stroke:#3f9b6a; }}
      .teacher-chart-legend {{ display:flex; align-items:center; gap:6px; font-size:11px; font-weight:700; color:#5f7185; }}
      .teacher-chart-legend span {{ width:16px; height:3px; border-radius:999px; display:inline-block; }}
      .teacher-chart-legend .actual {{ background:#3f7fb8; }}
      .teacher-chart-legend .schedule {{ background:#3f9b6a; }}
    ...
    <div class="panel teacher-status-panel" style="margin-top:8px">
      ...
      <form method="post" action="/teacher-attendance/update-times">
        <input type="hidden" name="date" value="{html.escape(selected_date)}">
        <input type="hidden" name="action" value="{html.escape(action_mode)}">
        <input type="hidden" name="pay_start" value="{html.escape(pay_start)}">
        <input type="hidden" name="pay_end" value="{html.escape(pay_end)}">
      <div class="table-wrap teacher-status-table">
        <table>
          <thead><tr><th>Teacher</th><th>Status</th><th>First in</th><th>Last out</th><th>SCHEDULE IN</th><th>SCHEDULE OUT</th><th>WORK HOURS</th></tr></thead>
          <tbody>{''.join(teacher_rows) or '<tr><td colspan="7" class="muted">No teachers found.</td></tr>'}</tbody>
        </table>
      </div>
        <div class="teacher-status-actions">
...
def render_closed_dates(user, query):
    ...
    rows = "".join(f"<tr><td>{html.escape(d)}</td><td><form method='post' action='/closed-dates/remove' onsubmit=\"return confirm('Remove this date?')\"><input type='hidden' name='date' value='{html.escape(d)}'><button class='btn red' type='submit'>Supprimer</button></form></td></tr>" for d in closed)
    body = f"""
    <div class="panel">
...
def render_users(user, query):
    ...
    with connect_db() as conn:
        users = conn.execute("SELECT * FROM web_users WHERE is_active = 1").fetchall()
        invite_people = conn.execute(
            """
            SELECT id, name, role, class_name
            FROM persons
            WHERE role IN ('children', 'teachers')
              AND id NOT IN (SELECT person_id FROM web_users WHERE person_id IS NOT NULL AND is_active = 1)
    ...
    def user_sort_value(target):
        ...
        if sort_key == "classes":
            return (", ".join(classes_value).lower(), (target["display_name"] or "").lower())
        if sort_key == "device":
    ...
    for target in users:
        ...
        if can_invite_user:
            ...
            invite_user_options.append(
                f'<option value="user:{target["id"]}"{selected_attr}>{html.escape(target["display_name"])} ({html.escape(ROLE_LABELS.get(target["role"], target["role"]))})</option>'
            )
        classes_value = json.loads(target["allowed_classes_json"] or "[]")
    ...
    if invite_user_options:
        invite_person_options.append(f'<optgroup label="Comptes utilisateurs">{''.join(invite_user_options)}</optgroup>')
    person_only_options = []
    for person in invite_people:
        ...
        person_only_options.append(
            f'<option value="person:{person["id"]}"{selected_attr}>{html.escape(person["name"])} ({role_label}{html.escape(class_text)})</option>'
        )
    if person_only_options:
        invite_person_options.append(f'<optgroup label="Personnes sans compte">{''.join(person_only_options)}</optgroup>')
    invite_rows = []
    ...
    if created_token:
        ...
        <div class="alert info" style="margin-top:12px">
          <div><strong>Invitation créée.</strong></div>
          <div style="margin-top:6px">Lien: <a href="{html.escape(created_invite_url)}" target="_blank">{html.escape(created_invite_url)}</a></div>
        </div>
        """
    users_panel_html = f"""
    <div class="panel" style="margin-top:16px">
      <h2>account</h2>
      <div class="muted">Créer, modifier, supprimer et inviter des utilisateurs.</div>
      ...
      <div class="table-wrap" style="margin-top:12px;max-height:420px;overflow:auto">
        <table>
          <thead><tr>
            ...
            <th class="sortable-head"><a href="/users?sort=username">Username</a></th>
            <th class="sortable-head"><a href="/users?sort=name">Name</a></th>
            <th class="sortable-head"><a href="/users?sort=role">Role</a></th>
            <th class="sortable-head"><a href="/users?sort=classes">Classes</a></th>
            <th class="sortable-head"><a href="/users?sort=device">Mobile device</a></th>
    ...
    <div class="panel" id="invite-user">
      ...
      <form method="post" action="/mobile-invitations/create" class="user-grid">
        ...
        <div><label>Personne</label><select name="invite_target" required>{''.join(invite_person_options)}</select></div>
    ...
    <style>
      ...
      .invite-records-scroll {{ max-height: 260px; overflow-y: auto; }}
      .invite-records-scroll table {{ margin: 0; }}
      .invite-records-scroll thead th {{ position: sticky; top: 0; z-index: 1; }}
    </style>
    {invite_panel_html}
    {users_panel_html}
    <script>
    ...
    """
    return html_page("INVITATIONS", user, body, flash=flash)
...
def render_children_admin(user, query):
    ...
    with connect_db() as conn:
        ...
        top_group_names = [row["class_name"] for row in conn.execute(
            ...
            FROM persons
            WHERE role = 'children' AND class_name <> ''
          ...
          <td class="nowrap">
            <div class="child-action-wrap" data-child-action-wrap>
              ...
              <div class="child-action-menu" data-child-action-menu="{child['id']}" aria-hidden="true">
                <a href="/children/edit?id={child['id']}">Modifier</a>
                <a href="/users?invite_person_id={child['id']}#invite-user">Inviter</a>
                <form method="post" action="/children/delete" onsubmit="return confirm('Delete this child and attendance records?')">
    ...
    <div class="panel">
      <div style="display:flex;align-items:center;justify-content:space-between;gap:12px;flex-wrap:wrap">
        <form method="post" action="/children/load-list" enctype="multipart/form-data" class="btn-row" style="align-items:end">
          <div>
            <label>Load list</label>
            <input type="file" name="child_list_file" accept="{schedule_file_accept_types()}" required>
          </div>
          <button class="btn primary" type="submit">Load list</button>
        </form>
      ...
      <div class="muted small" style="margin-top:10px">Total children: {total_children}</div>
      <form id="children-bulk-delete-form" method="post" action="/children/delete" style="display:none" onsubmit="return confirm('Delete the selected children and attendance records?')"></form>
      <div class="table-wrap" style="margin-top:12px;max-height:420px;overflow:auto">
        <table>
          <thead><tr><th style="width:34px;text-align:center"><input type="checkbox" id="children-select-all" aria-label="Select all children"></th><th class="sortable-head"><a href="/children?sort=name">Name</a></th><th class="sortable-head"><a href="/children?sort=group">Groupe</a></th><th>Photo path</th><th>Created</th><th>Action</th></tr></thead>
          <tbody>{rows or '<tr><td colspan="6" class="muted">No children found</td></tr>'}</tbody>
        </table>
      </div>
    </div>
    <div class="panel" style="margin-top:16px">
      ...
      <form method="post" action="/children/create" class="user-grid children-add-group">
        ...
        <div><label>Role</label>
          <select name="role">
            {"".join(f'<option value="{html.escape(role)}">{html.escape(ROLE_LABELS.get(role, role))}</option>' for role in creatable_roles_for_user(user))}
          </select>
        </div>
        <div><label>Groupe</label><input name="class_name" list="child-group-options" placeholder="Select or type a new group"></div>
        <datalist id="child-group-options">{class_options}</datalist>
        <div><label>New Groupe</label><input name="new_class_name" placeholder="Optional new group name"></div>
...
def render_mobile_invitations(user, query):
    ...
    selected_person_id = query.get("person_id", [""])[0]
    created_token = query.get("created", [""])[0]
    email_status = query.get("email", [""])[0]
    with connect_db() as conn:
        users = conn.execute("SELECT * FROM web_users WHERE is_active = 1 ORDER BY role, display_name").fetchall()
        people = conn.execute(
            """
            SELECT id, name, role, class_name
            FROM persons
        ...
        invitations = conn.execute(
            """
            SELECT mobile_invitations.*, persons.name AS person_name, persons.class_name AS class_name,
                   COALESCE(web_users.display_name, web_users.username, '') AS invited_by_name
            FROM mobile_invitations
            JOIN persons ON persons.id = mobile_invitations.person_id
            LEFT JOIN web_users ON web_users.id = mobile_invitations.invited_by_user_id
            ORDER BY mobile_invitations.id DESC
            LIMIT 100
            """
        ).fetchall()
    ...
    for target in users:
        ...
        if not can_invite_user:
            continue
        user_options.append(
            f'<option value="user:{target["id"]}">{html.escape(target["display_name"])} - @{html.escape(target["username"])} ({html.escape(ROLE_LABELS.get(target["role"], target["role"]))})</option>'
        )
    if user_options:
        person_options.append(f'<optgroup label="Comptes utilisateurs">{''.join(user_options)}</optgroup>')
    person_only_options = []
    for person in people:
        role_label = "Enfant" if person["role"] == "children" else "Educatrice"
        class_text = f" - {person['class_name']}" if person["class_name"] else ""
        selected = "selected" if str(person["id"]) == selected_person_id else ""
        person_only_options.append(
            f'<option value="person:{person["id"]}" {selected}>{html.escape(person["name"])} ({role_label}{html.escape(class_text)})</option>'
        )
    if person_only_options:
        person_options.append(f'<optgroup label="Personnes sans compte">{''.join(person_only_options)}</optgroup>')

    created_html = ""
    if created_token:
        invite_link = display_invite_url(created_token)
        if email_status == "sent":
            delivery_text = "Le MESSAGE a été envoyé."
        elif email_status == "error":
            delivery_text = "Le MESSAGE n'a pas pu être envoyé. Copiez le lien et envoyez-le manuellement."
        else:
            delivery_text = "SMTP n'est pas configuré. Copiez le lien et envoyez-le manuellement."
        created_html = f"""
        <div class="alert info">
          Invitation créée. {html.escape(delivery_text)}
          <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap;margin-top:8px">
            <input id="new-invite-link" value="{html.escape(invite_link)}" readonly>
            <button class="btn primary" type="button" data-copy-link="{html.escape(invite_link)}">Copier le lien</button>
          </div>
        </div>
        """
    ...
    for invite in invitations:
        invite_link = display_invite_url(invite["token"])
        status = "Acceptée" if invite["accepted_at"] else ("Expirée" if invitation_is_expired(invite) else "En attente")
        status_class = "present" if invite["accepted_at"] else ("warn" if invitation_is_expired(invite) else "absent")
        rows.append(
            f"""
            <tr>
              <td>{html.escape(invite['person_name'])}</td>
              <td>{html.escape('Enfant' if invite['role'] == 'children' else 'Educatrice')}</td>
              <td>{html.escape(invite['email'])}</td>
              <td>{html.escape(invite['created_at'])}</td>
              <td>{html.escape(invite['expires_at'])}</td>
              <td>{html.escape(invite['email_sent_at'] or '')}</td>
              <td>{html.escape(invite['accepted_at'] or '')}</td>
              <td><span class="badge {status_class}">{html.escape(status)}</span></td>
              <td>{html.escape(invite['email_error'] or '')}</td>
              <td class="nowrap">
                <button class="btn" type="button" data-copy-link="{html.escape(invite_link)}">Copier</button>
                <a class="btn ghost" href="{html.escape(invite_link)}" target="_blank">Ouvrir</a>
              </td>
            </tr>
            """
        )
    ...
    if user["role"] == "boss":
        location_panel = f"""
    <div class="panel" style="margin-top:16px">
      <h3>Lieu de travail mobile</h3>
      <div class="muted">Les présences mobiles des éducatrices doivent être faites dans ce rayon.</div>
      <form method="post" action="/mobile-location/update" class="user-grid" style="margin-top:12px">
        <div><label>Latitude</label><input id="mobile-location-lat" name="latitude" required value="{html.escape(str(location_policy.get('latitude') or ''))}"></div>
        <div><label>Longitude</label><input id="mobile-location-lon" name="longitude" required value="{html.escape(str(location_policy.get('longitude') or ''))}"></div>
    ...
    body = f"""
    <div class="panel">
      <h2>Invitations mobiles</h2>
      <div class="muted">Créez un lien d'inscription mobile pour un enfant ou une éducatrice. Le lien peut être envoyé par MESSAGE ou SMS.</div>
      {created_html}
      <form method="post" action="/mobile-invitations/create" class="user-grid" style="margin-top:12px">
        <div><label>Personne</label><select name="invite_target" required>{''.join(person_options)}</select></div>
        <div><label>MESSAGE</label><input name="email" type="email" required placeholder="nom@example.com"></div>
        <div><label>Valide pendant</label>
          <select name="days">
            <option value="7">7 jours</option>
        ...
        <div style="display:flex;align-items:end"><button class="btn primary" type="submit">Créer invitation</button></div>
      </form>
    </div>
    {location_panel}
    <div class="panel" style="margin-top:16px">
      ...
      <div class="table-wrap">
        <table>
          <thead><tr><th>Nom</th><th>Type</th><th>MESSAGE</th><th>Créée</th><th>Expire</th><th>MESSAGE envoyé</th><th>Acceptée</th><th>Statut</th><th>Erreur MESSAGE</th><th>Lien</th></tr></thead>
          <tbody>{''.join(rows) or '<tr><td colspan="10" class="muted">Aucune invitation</td></tr>'}</tbody>
        </table>
      </div>
    </div>
    <script>
    (function() {{
      document.addEventListener('click', function(event) {{
        ...
        window.setTimeout(function() {{ button.textContent = button.classList.contains('primary') ? 'Copier le lien' : 'Copier'; }}, 1400);
      }});
    ...
    """
    return html_page("Invitations mobiles", user, body)
...
def render_user_edit(user, target_id):
    ...
    password_html = f'<div id="password-reset"><label>Reset password</label><input name="password" type="password" placeholder="Leave blank to keep current" {"required" if self_edit or password_only else ""}></div>' if can_reset_password else ""
    body = f"""
    <div class="panel">
      <h2>Edit User</h2>
      ...
      <form method="post" action="/users/update" class="user-grid">
        ...
        <div><label>Role</label>
          <select name="role" {select_locked_attrs}>
            {''.join(f'<option value="{role}" {"selected" if role == target["role"] else ""}>{ROLE_LABELS.get(role, role)}</option>' for role in role_options)}
          </select>
        </div>
...
def json_response(handler, payload, status=200):
    handler.send_text(json.dumps(payload, ensure_ascii=False), status=status, content_type="application/json; charset=utf-8")
...
def smtp_config():
    ...
    from_address = os.environ.get(SMTP_FROM_ENV, "").strip() or username
    ...
    try:
        ...
        port = 587
    tls_value = os.environ.get(SMTP_TLS_ENV, "1").strip().lower()
    return {
        "host": host,
        "port": port,
        "username": username,
...
def send_mobile_invitation_email(to_address, person_name, invite_url, expires_at):
    subject = "Invitation mobile - Garderie L'Univers De Cassiopée"
    body = f"""Bonjour,

Vous avez reçu une invitation pour créer votre compte mobile Garderie L'Univers De Cassiopée.
...
def deliver_mobile_invitation_email(conn, handler, actor, token):
    ...
    if not invitation:
        return {"configured": bool(smtp_config()), "sent": False, "error": "Invitation not found"}
    if not smtp_config():
        conn.execute(
            "UPDATE mobile_invitations SET email_error = ? WHERE id = ?",
            ("SMTP is not configured", invitation["id"]),
        )
        return {"configured": False, "sent": False, "error": "SMTP is not configured"}
...
def ensure_mobile_person_for_user(conn, target_user):
    if target_user["person_id"]:
        person = conn.execute("SELECT * FROM persons WHERE id = ?", (target_user["person_id"],)).fetchone()
        if person:
            return person["id"]
    ...
    existing = conn.execute(
        "SELECT * FROM persons WHERE lower(name) = lower(?) AND role = 'teachers' ORDER BY id LIMIT 1",
        (name,),
    ).fetchone()
    if existing:
        ...
        conn.execute(
            ...
            VALUES (?, 'teachers', '', '', ?, ?)
            """,
            (name, f"STAFF:{secrets.token_urlsafe(16)}", now_text()),
        )
        person_id = conn.execute("SELECT last_insert_rowid()").fetchone()[0]
    conn.execute(
...
def create_mobile_invitation(conn, actor, person_id, email, days=7, role_override=None):
    ...
    conn.execute(
        """
        INSERT INTO mobile_invitations(token, person_id, role, email, invited_by_user_id, expires_at, accepted_at, created_at)
        VALUES (?, ?, ?, ?, ?, ?, NULL, ?)
        """,
        (token, person_id, role, email, actor["id"] if actor else None, expires_at, now_text()),
    )
    audit(
        conn,
        actor["id"] if actor else None,
...
def accept_mobile_invitation(conn, token, username, password):
    ...
    if existing:
        ...
        conn.execute(
            "UPDATE web_users SET person_id = ?, updated_at = ? WHERE id = ?",
            (invitation["person_id"], now_text(), existing["id"]),
        )
        user_id = existing["id"]
    else:
        class_names = get_classes(conn) if invitation["role"] in {"teacher", "principal"} else []
        user_id = create_user(
            conn,
            username=username,
...
def mobile_children_payload(conn, user, class_name="all"):
    classes = classes_for_user(user, conn)
    selected_class = (class_name or "all").strip() or "all"
    if selected_class not in classes and selected_class != "all":
        selected_class = classes[0] if classes else "all"
    children = get_children(conn, user, selected_class)
    today = today_text()
        ...
        "children": [
            {
                ...
                "status": current_child_status(conn, child["id"], today),
            }
            for child in children
        ],
    }
...
def mobile_person_status(conn, user):
    if not user["person_id"]:
        return None
    person = conn.execute("SELECT id, name, role, class_name FROM persons WHERE id = ?", (user["person_id"],)).fetchone()
    if not person:
        return None
    last = conn.execute(
        """
...
def attendance_location_payload(conn):
    row = conn.execute("SELECT * FROM attendance_locations WHERE id = 1").fetchone()
    if not row or row["latitude"] is None or row["longitude"] is None:
        return {"configured": False, "latitude": None, "longitude": None, "radius_meters": 100}
    return {
        "configured": True,
...
def validate_mobile_teacher_location(conn, latitude, longitude):
    policy = attendance_location_payload(conn)
    ...
    if distance > policy["radius_meters"]:
        raise ValueError(f"Outside work location radius ({int(distance)} m)")
    return {"distance_meters": round(distance, 1), "radius_meters": policy["radius_meters"]}
...
def pwa_manifest():
    return {
        "name": "Garderie L'Univers De Cassiopée",
        "short_name": "Garderie",
        "start_url": "/dashboard",
        "scope": "/",
        "display": "standalone",
...
<text x="256" y="455" text-anchor="middle" font-family="Arial, sans-serif" font-size="54" font-weight="800" fill="#24476f">GU</text>
</svg>"""
...
def health_payload():
    ...
    return {
        ...
        "counts": counts,
        "smtp_configured": bool(smtp_config()),
...
def render_mobile_install(user):
    ...
    <div class="panel" style="max-width:640px;margin:0 auto">
      <h2>Installer Garderie L'Univers De Cassiopée</h2>
      <p class="muted">Ajoutez cette application à l'écran d'accueil du téléphone. La prochaine fois, ouvrez l'icône Garderie L'Univers De Cassiopée sans saisir l'adresse du serveur.</p>
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == "/manifest.webmanifest":
            self.send_text(json.dumps(pwa_manifest(), ensure_ascii=False), content_type="application/manifest+json; charset=utf-8")
            return
        ...
        if path == "/api/mobile/me":
            ...
            return
        if path == "/api/mobile/children":
            mobile_user = get_bearer_session_user(self)
            if not mobile_user:
                json_response(self, {"ok": False, "error": "Unauthorized"}, status=401)
                return
            if mobile_user["role"] == "children":
                json_response(self, {"ok": False, "error": "Staff mobile account required"}, status=403)
                return
            with connect_db() as conn:
                payload = mobile_children_payload(conn, mobile_user, query.get("class", ["all"])[0])
        ...
        if path == "/api/mobile/location-policy":
            ...
            if not mobile_user:
                ...
                return
            with connect_db() as conn:
                payload = attendance_location_payload(conn)
        ...
        if path == "/api/messages/unread":
            ...
            return
        if user["role"] == "children" and path in {"/reports", "/statistics", "/statistics/15min", "/teacher-attendance", "/export.xlsx", "/contacts", "/contacts.xlsx", "/closed-dates", "/users", "/children", "/children/cards", "/mobile-invitations", "/audit", "/calendar", "/allergic-children"}:
            redirect(self, "/dashboard")
            return
        if path == "/dashboard":
    ...
    def do_POST(self):
        ...
        if path == "/api/mobile/login":
            ...
            with connect_db() as conn:
                ...
                conn.commit()
            json_response(self, {"ok": True, "token": token, "user": mobile_user_payload(target), "status": status})
            return
        if path == "/invite/accept":
            ...
            try:
                ...
                return
            redirect(self, "/mobile-install", {"Set-Cookie": session_cookie_header(session_token)})
            return
        if path == "/login":
            form = parse_post_data(self)
            username = form.get("username", [""])[0]
        ...
        if path == "/password-reset":
            ...
            recovery_contact = form.get("recovery_contact", [""])[0].strip()
            new_password = form.get("password", [""])[0]
            confirm = form.get("confirm", [""])[0]
            if len(new_password) < 8:
                self.send_html(render_password_reset(error="Password must be at least 8 characters."), status=200)
                return
            if new_password != confirm:
                self.send_html(render_password_reset(error="Passwords do not match."), status=200)
                return
            with connect_db() as conn:
                target = conn.execute(
                    "SELECT * FROM web_users WHERE lower(username) = lower(?) AND is_active = 1",
                ...
                profile = conn.execute("SELECT * FROM user_profiles WHERE user_id = ?", (target["id"],)).fetchone() if target else None
                if not target or not contact_matches_recovery(profile, recovery_contact):
                    self.send_html(render_password_reset(error="Username and recovery contact do not match."), status=200)
                    return
                try:
                    update_user_password(conn, target["id"], new_password)
                    audit_request(
                        self,
                        conn,
                        target["id"],
                        "password_reset_by_contact",
                        "user",
                        object_id=target["id"],
                        details={"username": target["username"], "recovery_method": "phone_or_email"},
                    )
                    conn.commit()
                except ValueError as exc:
                    self.send_html(render_password_reset(error=str(exc)), status=200)
                    return
            self.send_html(render_password_reset(success="Password reset. You can now log in."), status=200)
            return
        if path == "/logout":
            if user:
        ...
        if path == "/api/mobile/teacher-schedule":
            ...
            try:
                with connect_db() as conn:
                    linked_person = conn.execute("SELECT id, name FROM persons WHERE id = ? AND role = 'teachers'", (actor["person_id"],)).fetchone()
                    if not linked_person:
                    ...
                    schedule = teacher_schedule_for_day(conn, linked_person["name"], day_text)
        ...
        if path == "/api/mobile/teacher-face-attendance":
            ...
            try:
                with connect_db() as conn:
                    linked_person = conn.execute("SELECT * FROM persons WHERE id = ? AND role = 'teachers'", (actor["person_id"],)).fetchone()
                    if not linked_person:
                        raise ValueError("This account is not linked to a staff person")
                    device = validate_mobile_device(conn, actor, device_id, request_handler=self, device_name=device_name)
                    location_result = validate_mobile_teacher_location(conn, latitude, longitude)
        ...
        if path == "/api/mobile/location-policy":
            ...
            with connect_db() as conn:
                conn.execute(
                    """
                    INSERT INTO attendance_locations(id, latitude, longitude, radius_meters, updated_by_user_id, updated_at)
                    VALUES (1, ?, ?, ?, ?, ?)
                    ...
                    """,
                    (latitude, longitude, radius_meters, actor["id"], now_text()),
                )
                audit_request(
                    self,
                    conn,
            ...
            try:
                with connect_db() as conn:
                    ...
                    if target_user_id is not None:
                        target_user = conn.execute("SELECT * FROM web_users WHERE id = ?", (target_user_id,)).fetchone()
                        if not target_user:
                            raise ValueError("User not found")
            ...
            with connect_db() as conn:
                conn.execute(
                    ...
                    ON CONFLICT(id) DO UPDATE SET
                      ...
                      updated_at = excluded.updated_at
                    """,
                    (latitude, longitude, radius_meters, user["id"], now_text()),
                )
                audit_request(
                    self,
                    conn,
                    user["id"],
                    "update_mobile_location_policy_web",
                    "attendance_location",
                    object_id=1,
                    details={"latitude": latitude, "longitude": longitude, "radius_meters": radius_meters},
                )
                conn.commit()
            redirect(self, "/mobile-invitations")
            return
        ...
        if path == "/profile/calendar/delete":
            ...
            with connect_db() as conn:
                ...
                if target_user_id != user["id"] and not can_manage_users(user, target):
                    self.send_error(403)
                    return
                conn.execute("DELETE FROM child_calendar_events WHERE id = ?", (event_id_int,))
                audit_request(
                    self,
                    conn,
                    user["id"],
                    ...
                    "user",
                    object_id=target_user_id,
                    details={"event_id": event_id_int, "day_text": event["day_text"], "event_type": event["event_type"]},
                )
                conn.commit()
            redirect(self, f"/profile?user_id={target_user_id}&saved=1")
            return
        ...
        if path == "/mail/send":
            ...
            with connect_db() as conn:
                ...
                audit_request(self, conn, user["id"], "send_message", "internal_message", details={"recipients": [email for _rid, email in valid], "subject": subject, "attachments": sorted(set(saved_attachment_names))})
                conn.commit()
        ...
        if path == "/users/create":
            ...
            with connect_db() as conn:
                ...
                if not expected_person_role:
                    ...
                    if person_id:
                        person = conn.execute("SELECT * FROM persons WHERE id = ? AND role = ?", (person_id, expected_person_role)).fetchone()
                        if not person:
                            ...
                            return
                        existing_link = conn.execute("SELECT username FROM web_users WHERE person_id = ? AND is_active = 1", (person_id,)).fetchone()
                        ...
                        person_id = create_or_reuse_person_for_user_role(conn, role, display_name, class_name=class_name, photo_path=photo_path)
        ...
        if path == "/children/create":
            ...
            try:
                with connect_db() as conn:
                    if role == "children" and class_name:
                        conn.execute("INSERT OR IGNORE INTO class_names(name) VALUES (?)", (class_name,))
                    person_id = create_or_reuse_person_with_role(conn, role, name, class_name=class_name, photo_path=photo_path)
        ...
        if path == "/children/group/delete":
            ...
            if not group_name:
                ...
                return
            with connect_db() as conn:
                conn.execute("INSERT OR IGNORE INTO hidden_class_names(name) VALUES (?)", (group_name,))
                conn.execute("DELETE FROM class_names WHERE name = ?", (group_name,))
                audit_request(
                    self,
                    conn,
                    user["id"],
                ...
                conn.commit()
            redirect(self, "/children?group_deleted=1")
            return
        ...
        if path == "/children/load-list":
            ...
            if not entries:
                ...
                return
            token = store_pending_child_list_import(user["id"], upload.get("filename") or "", entries)
            redirect(self, f"/children?pending_import={quote(token)}")
            return
        ...
        if path == "/children/load-list/confirm":
            ...
            if not pending:
                ...
                return
            with connect_db() as conn:
                created, updated, skipped = apply_child_list_entries(conn, user["id"], pending["filename"], pending["entries"])
            redirect(self, f"/children?imported=1&created={created}&updated={updated}&skipped={skipped}")
            return
        ...
        if path == "/children/delete":
            ...
            with connect_db() as conn:
                ...
                for child_id in child_ids:
                    member = conn.execute("SELECT * FROM persons WHERE id = ?", (int(child_id),)).fetchone()
                    ...
                    linked_user = conn.execute("SELECT * FROM web_users WHERE person_id = ? AND is_active = 1", (int(child_id),)).fetchone()
                    if linked_user:
                    ...
                    conn.execute("DELETE FROM persons WHERE id = ?", (int(child_id),))
...
def child_calendar_panel_html(target, events):
    target_id = int(target["id"])
    today = date.today()
    month_start = today.replace(day=1)
    _, days_in_month = calendar.monthrange(month_start.year, month_start.month)
    weeks = calendar.Calendar(firstweekday=0).monthdatescalendar(month_start.year, month_start.month)
    week_head = "".join(f'<th style="width:88px;text-align:center;padding:5px 4px">{label}</th>' for label in ["Lun", "Mar", "Mer", "Jeu", "Ven", "Sam", "Dim"])
    events_by_day = {}
    for event in events:
    ...
    for week in weeks:
        ...
        for day in week:
            ...
            for event in day_events:
                ...
                event_badges.append(
                    f"""
                    <div style="margin-top:4px;border:1px solid var(--line);border-radius:6px;padding:3px;background:#f8fafc">
                      <div style="font-size:11px;font-weight:800">{html.escape(event['event_type'])}</div>
                      <div class="small muted" style="font-size:10px">{html.escape(note_text)}</div>
                      <form method="post" action="/profile/calendar/delete" onsubmit="return confirm('Supprimer cette date ?')" style="margin-top:3px">
                        <input type="hidden" name="target_user_id" value="{target_id}">
                        <input type="hidden" name="event_id" value="{event['id']}">
                        <button class="btn red" type="submit" style="min-height:24px;padding:2px 6px;font-size:11px">Supprimer</button>
                      </form>
                    </div>
                    """
                )
            cells.append(
                f"""
                <td class="child-calendar-day" data-date="{day_text}" style="width:88px;height:86px;vertical-align:top;padding:5px 4px;cursor:pointer;{outside_style}">
                  <div style="font-weight:900;font-size:16px;line-height:1">{day.day}</div>
                  {''.join(event_badges)}
                </td>
                """
            )
...
def render_child_home(user, query):
    ...
    <div class="grid" style="gap:16px">
      ...
      {child_calendar_panel_html(target, calendar_events)}
      <div class="panel">
        <h2>Files</h2>
...
def render_profile(user, query):
    ...
    if target["role"] == "children":
        ...
        calendar_html = child_calendar_panel_html(target, calendar_events)
    chooser = ""
    if len(users) > 1:
        options = "".join(
        ...
        <div class="mail-recipient-layout" data-recipient-picker>
          ...
          <div>
            ...
            <div class="mail-target-list" data-target-recipient>
              <div class="mail-target-head"><span>Rôle</span><span>Name</span><span>MESSAGE</span></div>
              {rows}
            </div>
            {group_hint}
          </div>
